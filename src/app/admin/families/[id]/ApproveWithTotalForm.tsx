    ...
    <details className="rounded-md border border-brand-pink/30 bg-brand-pink/5 p-3" open>
      ...
      <form action={action} className="mt-3 space-y-3">
        ...
        <div className="flex items-center gap-2">
          <label className="text-sm text-brand-ink/70">Total price ($):</label>
          <input name="total_amount" type="number" step="0.01" min="0" required className={`${inputClass} w-32`} />
        </div>
        <p className="text-xs text-brand-ink/50">
          Sets their tuition total and emails them to log in and choose quarterly vs paid-in-full
          themselves — no need to build out installments here. Use &quot;Create custom payment
          plan&quot; below instead if you need to set exact installment dates/amounts yourself.
        </p>
